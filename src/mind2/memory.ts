 ...
 * same pos and kind is refreshed (lastStock, seenTick); otherwise a new entry
 * is appended. Building entries always carry lastStock = 1. After each
 * append, entries are evicted (smallest seenTick first, ties by pos.x, pos.y,
 * kind) until length <= MEMORY_CAP.
 */
export function updateMemory(npc: W2NpcState, obs: W2Observation): void {
  const memory = npc.memory;

  function upsert(kind: ResourceKind | BuildingKind, pos: Vec2, lastStock: number, seenTick: number): void {
    ...
    memory.push({ kind, pos: { x: pos.x, y: pos.y }, lastStock, seenTick });
  ...
  for (const s of sites) upsert(s.kind, s.pos, s.stock, obs.tick);

  const buildings = [...obs.visibleBuildings].sort((a, b) => cmpStr(a.id, b.id));
  for (const b of buildings) upsert(b.kind, b.pos, 1, obs.tick);
}
...
export function inheritMemory(
  ...
  return top.map((entry, i) => {
    ...
    return { kind: entry.kind, pos, lastStock: entry.lastStock, seenTick: tick };
  });
}
