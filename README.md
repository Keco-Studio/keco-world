...
- `npm run formal -- run --arm <random|fixed|handcrafted|evolutionary|noculture> [--seeds 12] [--ticks 50000] [--chunk 1000] [--out runs/formal] [--seed-prefix pilot]` — the formal 1C runner (§17.1 step 9): chunked long run of one arm, resumable at per-seed granularity, archives the full artifact set per seed under `<out>/<arm>/<seedRoot>/`; see `docs/prereg-1c-draft.md` §1/§7
- `npm run formal -- gates --arm <id> [--out runs/formal]` — evaluate the S1–S5 survival/degradation gates over an archived arm, write `<out>/sgates-<arm>.json`; see `docs/prereg-1c-draft.md` §2
- `npm run formal -- novelty --arm <evolutionary|noculture> [--out runs/formal]` — evaluate the N1–N3 behavioral-novelty gates (requires the `fixed` arm archived under the same `--out` for the N3 baseline), write `<out>/novelty-<arm>.json`; see `docs/prereg-1c-draft.md` §3
- `npm run evalpack -- [--out runs/formal] [--arms evolutionary,handcrafted] [--pairs 25] [--packet runs/evalpack] [--seed evalpack-v1]` — build the blinded judge packet: samples/pairs/blinds candidate biographies from the archived Evolutionary and Handcrafted arms, hard-errors on any blinding violation, writes `packet.html` (printable questionnaire) and a separate `answer-key.json` (never bundled into the packet); see `docs/prereg-1c-draft.md` §4
- `npm run analyze -- --out runs/formal [--judgments <csv>] [--answer-key runs/evalpack/answer-key.json]` — the analysis master: aggregates every arm's S/N-gate reports (reads `sgates-<arm>.json`/`novelty-<arm>.json` when present, computes on miss) and, when `--judgments` (a `pairId,judgeId,choice` CSV, `choice` ∈ `left|right`) is supplied, joins it against the eval packet's answer key to compute the primary-endpoint statistics — exact two-sided binomial test, Wilson CI, cluster-robust standard error — and prints an advisory Go/Iterate/Stop recommendation per §6 (the actual decision stays human); writes `<out>/analysis.json`; see `docs/prereg-1c-draft.md` §4–§6, `docs/eval-rubric.md`
...
- `docs/prereg-1c-draft.md` — 1C formal-verification preregistration draft (§17.1 step 9): five arms, S/N hard gates, primary/secondary endpoints, archive policy, Go/Iterate/Stop decision rule; **draft, not yet frozen** — see the file's own freeze conditions
- `docs/eval-rubric.md` — secondary-endpoint scoring rubrics (recall accuracy, 0–3 causal-retelling completeness, conversion), double-rater protocol with Cohen's κ ≥ 0.6 gate
