...
- **磁盘重放 CLI（`src/cli/replay.ts`）尚未持久化/加载 `patronDirectives`**——**已关闭。** `src/cli/run.ts` 现在始终写出 `directives.json`（守望指令的规范排序形式：按 `tick` 升序、同拍内按 `npcId` 的 UTF-16 序升序；无指令的运行写 `[]`），使每个运行目录对自己的守望指令保持显式。`src/cli/replay.ts` 加载时若该文件存在则读入并传给 `verifyReplay`/`verifyStrict`；不存在则视为 `undefined`，不报错——本次交付之前写出的运行目录（没有 `directives.json`）仍可正常校验。用过守望指令的运行现在可以端到端走磁盘重放 CLI 校验，不再局限于进程内 API（`tests/run-replay-cli.test.ts`）。
- 守望审计字段（`patronInfluence`/`patronDecisive`，与 `actionSource` 同类）是标注字段：不进入 `WorldState`，注入式重放会原样采信日志里的值。事件哈希链能抓住对个别事件的篡改；但若攻击者只翻转标注位并把整条哈希链重新拼接，注入式重放会静默通过——checkpoint 哈希对此无感（标注不影响状态）；提供 `patronDirectives` 也帮不上——directives 只影响状态与活体决策路径，对注入式重放里被原样采信的标注位没有校验作用（漏提供 directives 会因 `patronThemes` 状态发散而报错，但那是另一类失败，不是对标注翻转的检测）。补救在于内核的完全确定性：用同一 manifest/roster/seed/`patronDirectives` 做一次**不注入的活体重演**（`runSim` 无 `injectedActions`），引擎会重新算出全部决策与标注位，生成可逐字节对照的真值日志——任何标注翻转都会在对照中现形。**已关闭：** `src/cli/replay.ts` 新增 `--strict` 参数，调用 `verifyStrict`，失败时打印首个分歧事件的 index/tick（`tests/strict-verify.test.ts`、`tests/run-replay-cli.test.ts`）。

  至此重放 CLI 输出两个独立、互补的验证结论：**注入式重放**（`replay`，始终运行）校验**世界轨迹忠实性**——把日志"注入"回引擎重演状态，同时做**完整性定位与篡改局域化**（截断/篡改会被定位到具体 tick），代价小、速度快；**严格模式**（`strict`，`--strict` 开启）校验**注记真值**——即 `actionSource`/`patronInfluence`/`patronDecisive` 的重算对照，能抓住"翻转标注位后重新拼接哈希链"这类注入式重放无法察觉的篡改，代价是完整重跑一遍无注入的活体模拟。日常校验（CI、常规重放）用注入式（快）；审计争议（对某条记录的标注真实性有异议）用严格（权威）。
