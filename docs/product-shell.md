...
- **双重审计**：`patronApplied`（倾斜确实进入了一次抽签）与 `patronDecisive`（结果与"无守望影子对照"不同——即结果因为倾斜而改变）。行动日志的 `patronInfluence` 字段（schema v4 新增必填字段）记录 `patronApplied`；`DecideInfo` 同时携带两个标志。为什么卡片的决胜标注（"它犹豫时，你的守望让它倾向了{verb}"）只在 `patronDecisive` 为真时出现，且可回溯到这个审计字段——不是凭感觉展示。
...
- **磁盘重放 CLI（`src/cli/replay.ts`）尚未持久化/加载 `patronDirectives`**——用过守望指令的运行目前只能通过进程内的 `verifyReplay(..., patronDirectives)` API 验证重放一致性（`tests/patron.test.ts` 走的正是这条路径），无法端到端走磁盘重放 CLI。这是一个已知的后续任务，不阻塞本次交付（守望指令仍会哈希进状态、行动日志里的 `patronInfluence` 仍会随内存重放校验）。
