...
> 1. 项目所有者审定本方案全部参数（尤其"待拍板"清单）——**唯一未满足项**；
> 2. ~~运行与分析代码全部落库（见 §9 交付物清单）并通过评审~~ **已满足**（2026-07-24，终审 READY）；
> 3. ~~形成性小样（非官方 seed）跑通全流程~~ **已满足**（2026-07-24，全尺度 5 臂 × 12 seed × 50k tick，见 docs/pilot-1c.md——含三项直接影响 §10 决策的实证发现）。
...
- **素材生成（机械规则，冻结）**：
  ...
  - **跨代分层采样（修正传记截断偏置）**：把血脉存续代数等分为 4 段，12 条成员事件预算按 3/段分配；段内优先级 死亡(带死因) > belief_formed > 出生，同级取更早 tick；某段不足时余额顺延后段；结语在评审包渲染中替换为仅按血脉存续状态二态的中性收尾（产品端渲染保留漂移结语）——原漂移结语在 clone/breed 臂间是决定论的机制指纹（终审发现，2026-07-24），漂移短语已入禁用词表；
  - **配对**：按代数带（如 1–15/16–30/31+）匹配 Evolutionary×Handcrafted 各一条，渲染字符长度差 ≤20%（超出则重抽该臂候选）；
  - **盲化核查表**：无 seed/tick/臂名/机制词；两臂共用 NAME_POOL，名字不泄臂；渲染器输出禁用词 grep 过检（算力/代币/模型/锦标赛/LoRA/世界进化 + random/fixed/handcrafted/evolutionary）；**信念句对称性**：若传记含"信念"类语句（如"它相信……"/"它认为……"），两臂必须用**同一模板寄存器**（措辞结构、出现位置、密度）呈现——不能只有一臂的传记带信念句、另一臂没有，因为"是否出现信念句"本身就是机制指纹，比任何禁用词都更隐蔽的去盲泄露点（代码评审中发现的具体去盲漏洞，2026-07-24 补记）。落地说明：评审包渲染统一为无时间戳的『信奉』单模板（产品端渲染不受此约束）。残余风险如实声明：信念句密度（进化臂跨成员多条 vs 手工臂仅始祖 ≤3 条）与措辞池多样性（内核 9 变体 vs 设计 20 条）仍是统计层面的潜在指纹，属形成性去盲探针要专门测量的对象，冻结时由项目所有者决定接受或进一步收紧。
...
- [ ] 中性结语替换是否接受（或整段删除结语）——评审包渲染已把结语从漂移语义改为仅按存续状态二态的中性收尾（"这一脉的故事仍在继续/至此落幕"），项目所有者需确认这一替换本身是否足够中性，或是否应干脆整段删除结语而非保留任何收尾句。
